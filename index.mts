...
import { datahandler, ExtractedData } from './lib/datahandler.mjs';
 ...
 * @param data 
 */
function aftersoil(rawdata: ExtractedData[]) {
...
async function main() {
    ...
    if (existsSync(jsonpath)) {
        const loaddata: ExtractedData[] = JSON.parse(String(readFileSync(jsonpath)));
        let baseflag = 1;
        loaddata.map((data) => {
            let flag = 1
            listdata.map((data2) => {
