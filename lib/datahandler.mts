...
// const url = 'https://e-words.jp/w/DDoS%E6%94%BB%E6%92%83.html';
// const outputFile = 'ddos_attack_page.html';
//返り値　形制定
//code -1はエラー
...
export class datahandler {
     ...
     * @param ursstr 単語
     */
    public generateURLS(ursstr: string[]) {
     ...
     * @returns 
     */
    public async scrapeURLs(urls: string[], lim: number) {
        return new Promise<ExtractedData[]>(async (resolve) => {
            //渡されたURLSを基に、promice allとPlimitで引っ掛けまくる
            const limit = pLimit(lim);
            const tasks: Promise<ExtractedData>[] = [];
            urls.map((inv) => {
     ...
     */
    private async scrapeWebsite(url: string): Promise<ExtractedData> {
        return new Promise<ExtractedData>(async (resolve) => {
            const whybuglog = "\n" + url + "を要求...";
            console.log(whybuglog);
            if (!this.browser) await this.initbrowther();
            if (this.browser) {
     ...
     */
    public gotmoreinfo(data: ExtractedData[]) {
         ...
         * @returns 
         */
        function removeengwords(data: ExtractedData[]): ExtractedData[] {
            function removeBracketsContent(text: string): string[] {
                return text.replace(/【[^】]+】/g, '').trim().split(" ");
            }
            let returndata: ExtractedData[] = [];
            data.map((data2) => {
         ...
         * @returns 
         */
        function getinformation(data: ExtractedData[]) {
            const returndata: ExtractedData[] = [];
            data.map((data) => {
                let infor = "";
